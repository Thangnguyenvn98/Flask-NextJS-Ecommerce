...
export const CategoryForm: React.FC<CategoryFormProps> = ({initialData,billboards}) => {
    ...
    const onSubmit = async (data: CategoryFormValues) => {
        try {
            ...
            if(initialData){
                await axios.patch(`http://127.0.0.1:8080/api/${params.storeId}/categories/${params.categoryId}`,datas)
               
            } else {
                await axios.post(`http://127.0.0.1:8080/api/${params.storeId}/categories`,datas)
    ...
    const onDelete = async () => {
        try {
            setLoading(true)
            await axios.delete(`http://127.0.0.1:8080/api/${userId}/${params.storeId}/category/${params.categoryId}`)
            router.push(`/${params.storeId}/categories`)
            router.refresh()
